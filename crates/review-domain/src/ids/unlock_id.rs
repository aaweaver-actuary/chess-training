...
/// Strongly typed identifier for unlock records associated with learners.
///
/// ```
/// use review_domain::ids::{UnlockId, IdConversionError, IdKind};
///
/// let id = UnlockId::try_from(7_u128).unwrap();
/// assert_eq!(id.get(), 7);
///
...
impl From<UnlockId> for u64 {
    /// Returns the raw `u64` backing this identifier.
    fn from(value: UnlockId) -> Self {
        value.0
    }
}

impl TryFrom<u128> for UnlockId {
...
impl TryFrom<i128> for UnlockId {
    type Error = IdConversionError;
...
impl fmt::Display for UnlockId {
    /// Formats the identifier for display purposes.
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "UnlockId({})", self.0)
    }
}
