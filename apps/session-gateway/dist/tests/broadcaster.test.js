...
describe('broadcaster', () => {
    it('delivers messages to registered sockets and stops after unregistering', () => {
        ...
        broadcaster.register('session', socket);
        broadcaster.register('session', socket);
        broadcaster.register('session', second);
        ...
        broadcaster.unregister('session', socket);
        broadcaster.broadcast('session', { ignored: true });
        expect(socket.messages).toHaveLength(1);
        expect(second.messages).toHaveLength(2);
        broadcaster.unregister('session', second);
    ...
    it('ignores sockets that are not open', () => {
        ...
        const broadcaster = createBroadcaster();
        broadcaster.register('session', closed);
    ...
    it('safely handles unregister calls for unknown sessions', () => {
        const broadcaster = createBroadcaster();
        const socket = new FakeSocket();
        expect(() => broadcaster.unregister('missing', socket)).not.toThrow();
    });
});
