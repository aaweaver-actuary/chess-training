...
const createClient = (fetchImpl) => createHttpSchedulerClient({ baseUrl: 'http://scheduler.test', fetchImpl });
const sampleCard = {
    card_id: 'card-1',
    kind: 'Opening',
    position_fen: 'start',
...
describe('http scheduler client', () => {
    it('fetches queues and next cards from scheduler', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith('http://scheduler.test/queue', expect.anything());
    ...
    it('throws when scheduler responds with non-ok status', async () => {
        ...
        await expect(client.gradeCard({ sessionId: 's', cardId: 'c', grade: 'Again', latencyMs: 10 })).rejects.toThrow('scheduler-error');
    });
    it('falls back to global fetch implementation when none is provided', async () => {
        const originalFetch = global.fetch;
        const fetchSpy = vi.fn(async () => new Response(JSON.stringify({ queue: [] }), { status: 200 }));
