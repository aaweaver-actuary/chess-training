...
export const createGatewayServer = (deps) => {
    ...
    app.post('/api/session/start', asyncHandler(async (req, res) => {
        const parsed = startSchema.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json(jsonError('BAD_REQUEST', 'Invalid user_id'));
            return;
        }
        const { sessionId, queueSize, firstCard } = await service.start(parsed.data.user_id);
        res.json({ session_id: sessionId, queue_size: queueSize, first_card: firstCard });
    }));
    app.post('/api/session/grade', asyncHandler(async (req, res) => {
    ...
    app.get('/api/session/stats', asyncHandler(async (req, res) => {
