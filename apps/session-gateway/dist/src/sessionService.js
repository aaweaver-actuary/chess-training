...
const applyGrade = (current, nextCard, grade, latencyMs) => {
    ...
    const accuracy = computeAccuracy(correctReviews + (isCorrect(grade) ? 1 : 0), totalReviews);
...
export const createSessionService = (deps) => {
    const { schedulerClient, sessionStore, broadcaster, logger = { info: noop, warn: noop, error: noop }, } = deps;
    return {
        async start(userId) {
        ...
        async grade({ sessionId, cardId, grade, latencyMs }) {
            ...
            const updated = await sessionStore.update(sessionId, (state) => applyGrade(state, nextCard, grade, latencyMs));
